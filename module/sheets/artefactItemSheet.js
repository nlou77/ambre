console.log("Ambre | artefactItemSheet.js SCRIPT LOADED"); // <-- ADD THIS LINE

export default class ArtefactItemSheet extends ItemSheet {
    constructor(...args) {
        super(...args);
...
return foundry.utils.mergeObject(super.defaultOptions, {
        classes: ["ambre", "sheet", "item", "artefact-card-sheet"],
        template: "systems/ambre/templates/sheets/artefact-sheet.hbs", // Corrected path
    ...
    get template() {
        return `systems/ambre/templates/sheets/artefact-sheet.hbs`;
    }
    ...
    async getData(options) {
        ...
        context.system.category = Number(item.system.category) || 0; // Ensure category is a number, default 0

        // Prepare category button data
        const category = context.system.category;
        const categories = [
            { value: 0, icon: "fas fa-cube", tooltip: "Objet Basique", theme: "theme-defaut" },
            { value: 1, icon: "fas fa-gem", tooltip: "Artefact (Pentacre)", theme: "theme-pentacre" },
            { value: 2, icon: "fas fa-skull-crossbones", tooltip: "Artefact Abyssal", theme: "theme-abysses" },
            { value: 3, icon: "fas fa-flask", tooltip: "Ingrédient Alchimique", theme: "theme-defaut" }
        ];
        context.categoryButton = categories.find(c => c.value === category) || categories[0];
        // Ensure a fallback if category value is somehow out of bounds
        if (!categories.find(c => c.value === category)) {
            context.system.category = 0; // Reset to default if invalid
        }
        ...
        //console.log(`Ambre | getData for Artefact: "${item.name}" (ID: ${item.id}, UUID: ${item.uuid}, isOwned: ${item.isOwned})`);
    ...
    activateListeners(html) {
        ...
        if (this.item.system.locked) {
            html.find('.attribute-level-select').each((_index, selectElement) => {
                ...
                if (select.val() == "0" || select.val() == 0) {
                    const gridContainer = select.closest('.artefact-attributes-list');
        ...
        const panel = html.find('.artefact-side-panel');
        const toggleBtn = html.find('.toggle-panel-btn');
        const mainContent = html.find('.artefact-main-content'); // To push content

        const leftColumn = html.find('.artefact-left-column');
        const leftColumnToggleBtn = html.find('.toggle-left-center-column-btn');
        const leftColumnToggleIcon = leftColumnToggleBtn.find('i');
        const leftColumnWidth = 250; // Correction: Doit correspondre à flex-basis de .artefact-left-column dans CSS

    
        // Restore panel state if it was previously saved
        if (this.item.getFlag("ambre", "artefactPanelOpen")) {
            panel.addClass('open');
        ...
        toggleBtn.on('click', () => {
            ...
            // Save panel state
            this.item.setFlag("ambre", "artefactPanelOpen", !isOpen);
        });

        // --- Left Column Toggle (between Left and Center) ---
        let isLeftColumnVisible = this.item.getFlag("ambre", "artefactLeftColumnVisible") || false; // Default to hidden
        ...
        leftColumnToggleBtn.on('click', () => {
            ...
            this.item.setFlag("ambre", "artefactLeftColumnVisible", newVisibility);
        });

        // --- Collapsible Sections State Management ---
        const collapsibleHeaders = html.find('.artefact-collapsible-header');
        ...
        html.find('#artefact-polymorphie-content-center .polymorphie-main-value .artefact-attribute-value-control .value-increment').click(async ev => {
        ...
        html.find('#artefact-polymorphie-content-center .polymorphie-main-value .artefact-attribute-value-control .value-decrement').click(async ev => {
            const currentValue = parseInt(this.item.system.polymorphie.value) || 0;
        ...
        html.find('.refresh-possessing-actor').click(async ev => {
            ev.preventDefault();
            console.log(`Ambre | 'Refresh & Lock' button clicked for Artefact: "${this.item.name}".`);
            if (this.item.system.locked) {
                ui.notifications.warn("L'action de rafraîchissement est désactivée car l'artefact est verrouillé.");
                console.log(`Ambre |   Action aborted: Artefact is already locked.`);
                return;
            }
            ...
            let notifMsg = `Artefact "${this.item.name}" verrouillé. `;
        ...
        html.find('.toggle-stat-refresh-lock').click(async ev => {
            ev.preventDefault();
            if (this.item.system.locked) {
                ui.notifications.warn("L'artefact est verrouillé. Déverrouillez-le pour changer ce paramètre d'actualisation des stats.");
                return;
            }
        ...
        html.find('.toggle-artefact-lock').click(async ev => {
            ev.preventDefault();
            const currentLockState = this.item.system.locked || false;
            console.log(`Ambre | 'Toggle Lock/Unlock' icon clicked for Artefact: "${this.item.name}". Current lock state: ${currentLockState}.`);
            if (currentLockState) { // Is currently locked, about to unlock
                await this.item.update({ "system.locked": false });
                ui.notifications.info(`Artefact "${this.item.name}" déverrouillé. Les statistiques précédemment stockées sont préservées.`);
                console.log(`Ambre |   Artefact was locked, now unlocked. Previously stored stats are preserved.`);
            } else {
                ...
                ui.notifications.info(`Artefact "${this.item.name}" verrouillé. Les statistiques précédemment stockées (le cas échéant) ont été préservées.`);
                console.log(`Ambre |   Artefact is unlocked, now locked. Previously stored stats (if any) are preserved. Stat refresh logic NOT invoked by this action.`);
            }
        });

        // Listener for the category button using event delegation
        // Attach listener to a more static parent (html, which is this.element.find('.sheet-body') or similar)
        html.on('click', '.category-button', async ev => {
            ev.preventDefault(); // Ensure this is called first
            if (this.item.system.locked) {
                ui.notifications.warn("L'artefact est verrouillé. Déverrouillez-le pour changer sa catégorie.");
                return;
            }
            let currentCategory = this.item.system.category || 0; // Read from item directly
            currentCategory = (currentCategory + 1) % 4; // Cycle 0, 1, 2, 3
            //await this.item.update({"system.category": currentCategory});
            //this._applyThemeBasedOnCategory();
            //this.render(true); // Render the sheet again to reflect the change
        });                        
        ...
        html.find('.clear-possessing-actor-on-lock').click(async ev => {
            ...
            if (this.item.system.locked) {
                ui.notifications.warn("L'artefact doit être déverrouillé pour oublier les données stockées.");
                return;
            }
            if (this.item.system.possessingActorNameOnLock) {
                const confirmed = await Dialog.confirm({
                    title: "Confirmer l'oubli",
                    content: `<p>Voulez-vous vraiment oublier les données de l'acteur <strong>${this.item.system.possessingActorNameOnLock}</strong> (précédemment stockées) pour cet artefact ? Les statistiques (Physique, Endurance, etc.) associées seront également effacées.</p>`,
                    yes: () => true,
                    no: () => false,
                    defaultYes: false
                });
    ...
    async _handleStatRefreshLogic() {
        console.log(`Ambre | _handleStatRefreshLogic called for Artefact: "${this.item.name}".`);
    ...
    /**
     * Applies the correct theme class to the item sheet window based on its category.
     * @private
     */
    _applyThemeBasedOnCategory() {
        if (!this.element || !this.element.length) return;

        const windowApp = this.element.closest('.window-app.app.item-sheet');
        if (!windowApp.length) return;

        const currentCategory = Number(this.item.system.category) || 0;
        const categories = [
            { value: 0, theme: "theme-defaut" },
            { value: 1, theme: "theme-pentacre" },
            { value: 2, theme: "theme-abysses" },
            { value: 3, theme: "theme-defaut" }
        ];

        const newTheme = categories.find(c => c.value === currentCategory)?.theme || "theme-defaut";

        // Remove all potential theme classes
        categories.forEach(cat => windowApp.removeClass(cat.theme));
        // Also remove generic theme classes if they were applied this way
        windowApp.removeClass('theme-dark theme-light theme-fantasy');

        // Add the new theme class
        windowApp.addClass(newTheme);
        console.log(`Ambre | ArtefactItemSheet: Applied theme "${newTheme}" for category ${currentCategory}.`);
    }
